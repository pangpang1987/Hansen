...
def create_population(
    varTypes, # problem.varTypes = np.array(varTypes)
    ranges,   # problem.ranges = np.array([lb, ub])  # 初始化ranges（决策变量范围矩阵）
    borders,   # problem.borders = np.array([lbin, ubin])  # 初始化borders（决策变量范围边界矩阵）
    NIND,               # 种群规模，即一代所需个体数
    Encoding='RI'       # 编码类型, 默认实型
) :     # class
    ...
    population = ea.Population(Encoding, Field, NIND)           # 实例化种群对象列表
