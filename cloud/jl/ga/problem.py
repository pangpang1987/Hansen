...
class Myproblem(ea.Problem) :
    ...
    def __init__(
        self,
        name,                           # str, 问题名称，可以自由设置
        M,                              # int, 目标维数, 即有多少个优化目标
        maxormins,                      # array, 目标函数最小最大化标记的行向量.
                                        # 1表示最小化，-1表示最大化。例如：array([1,1,-1,-1]), 
                                        # 表示前两个目标是最小化, 后两个是最大化.
        Dim,                            # int, 决策变量维数，即有多少个决策变量.
        varTypes,                       # array, 连续或离散标记，是Numpy array类型的行向量,
                                        # 0表示对应的决策变量是连续的; 1表示对应的变量是离散的。
        lb,                             # self.ranges = np.array([lb, ub]), 决策变量范围矩阵,
                                        # 第一行对应决策变量的下界
        ub,                             # 第二行对应决策变量上界
        lbin,                           # self.borders = np.array([lbin, ubin])，决策变量范围的边界矩阵,
                                        # 第一行对应决策变量的下边界，1表示包含，0表示不包含  
        ubin,                           # 第二行对应决策变量的上边界，1表示包含，0表示不包含
        aimFunc             =   None,   # aimFunc(pop) : 目标函数，需要在继承类即自定义的问题类中实现，或是传入已实现的函数。
        ...
        calReferObjV        =   None,   # 计算目标函数参考值，需要在继承类中实现，或是传入已实现的函数。
                                        # ReferObjV : array - 存储着目标函数参考值的矩阵，每一行对应一组目标函数参考值，每一列对应一个目标函数。
                                        # self.ReferObjV = self.getReferObjV()  # 计算目标函数参考值
                                        # getReferObjV()   : 获取目标函数参考值。
        CustomAimFunc       =   None,   # 自定义适应值函数
        data                =   None,   # 外部数据
        # FuncProcess         =   None,   # 多进程适应函数
        # FuncThread          =   None,   # 多线程适应函数
        # FuncSerial          =   None,   # 串行适应函数
        # CreateDataParallel  =   None,   # 创建并行数据函数, 将r装入
        fitnesss            =   None,   # 全部个体适应值
        poolType            =   None  
        ...
        # 外部导入
        self.CustomAimFunc      =   CustomAimFunc       # 自定义适应函数
        self.data               =   data                # 外部数据
        # self.FuncProcess        =   FuncProcess         # 多进程适应函数
        # self.FuncThread         =   FuncThread          # 多线程适应函数
        # self.FuncSerial         =   FuncSerial          # 串行适应函数
        # self.CreateDataParallel =   CreateDataParallel  # 创建种群函数
        self.fitnesss           =   fitnesss            # 全部个体适应值
        self.poolType           =   poolType  

    #€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€
    #   重载适应函数
    def aimFunc(self,pop) :
        self.CustomAimFunc(             # 外部自定义适应函数
            pop,                        # 染色体种群
            # self.CreateDataParallel,    # 创建并行数据, 与种群对应
            # self.FuncProcess,           # 多进程函数
            # self.FuncThread,            # 多线程函数
            self.fitnesss,              # 种群适应值
            self.data,                  # 外部数据
            self.poolType               # 并行类型
        )
