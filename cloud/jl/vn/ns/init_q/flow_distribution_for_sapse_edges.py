...
def input(edges_dict, low_l_1):
    ...
    for i in range(len(s_list)):
        ...
        # u = target_list(edges_list, t='u')[i]
...
def Dinic(edges_dict, low_l=0.5, n=6):
    """
    计算图的边有上下界容量问题的可行流分配
    ...
    # 计算最大流，验证满流
    # 不能设置异常中断，否则排序、平差模式不能进行
    flag = True
    for a, b, f, w in ans[1]:
        if a == Datasets['SS'] and abs(f - w) > (1 / 10**n):
            flag = False
            # print(flag)
            # return flag, None       # 不平衡  负风量不适应，误差总有非常大
            # message = 'Air volume initialization error!'
    ...
    # print(output)
    initQs = {**{e['id']:e['Q'] for e in output["edges"]}}
