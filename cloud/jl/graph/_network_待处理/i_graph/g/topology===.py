...
def get_topology(edges) -> dict :
    ...
    return {     # 常用拓扑关系
        ...
        'sourceNodes'   :   [g.vs[v.index]['id'] for v in g.vs if g.degree(v, mode='in')==0], # list, [vid, ...], 源点id列表
        'sinkNodes'     :   [g.vs[v.index]['id'] for v in g.vs if g.degree(v, mode='out')==0],             # list, [vid, ...], 汇点id列表
        'sourceEdges'   :   [g.es[ei]['id'] for vi in g.vs if g.degree(vi, mode='in')==0 for ei in g.incident(vi,mode='out')],
        'sinkEdges'     :   [g.es[ei]['id'] for vi in g.vs if g.degree(vi, mode='out')==0 for ei in g.incident(vi,mode='in')],
